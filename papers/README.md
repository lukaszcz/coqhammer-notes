# Reference papers and archived sources

This directory contains plain-text extracts of the papers referenced in
`../extraction.md`, `../monomorphisation.md`, `../type_guards.md` and
`../dependent_types.md`.
...
## Indexed-family archive

Archived papers for `notes/indexed_families.md`; related research reports are
in `../research/`. Each paper is stored as the
original PDF plus a `pdftotext -layout` conversion (`.txt`) for agent-readable
access. Formulas in the `.txt` files may be garbled where PDFs use symbol
fonts; consult the PDF for exact statements.
...
| `sozeau-etal-2020-coq-coq-correct` | M. Sozeau, S. Boulier, Y. Forster, T. Tabareau, T. Winterhalter, "Coq Coq Correct! Verification of Type Checking and Erasure for Coq, in Coq", POPL 2020 (PACMPL 4). |
...
Line numbers may drift; grep for section titles.
