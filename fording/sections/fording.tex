...
strictly worse: an \emph{unguarded} inversion axiom is not merely
false in the intended model --- its index equations force any two
index tuples of the same element to coincide, which for arbitrary
tuples collapses the universe to a point, so the axiom is inconsistent
with the family's own injectivity and discrimination axioms as soon as
the environment declares \emph{any} two-constructor datatype
(\cref{prop:unguarded-inconsistent}).  For families that specialise an
index to a constructor form --- vectors, finite sets, and their
relatives --- discrimination alone detonates it, and
enumeration-shaped families detonate by cardinality as well
