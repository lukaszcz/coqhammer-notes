...
\[
\begin{array}{c}
\rul{I : (\sigma_1, \dots, \sigma_m) \to \Set \in \Env \quad
     \Gamma \vdash a_h : \sigma_h \ (1 \le h \le m)}
...
\caption{Typing of terms, part II.  In the $\tscase{J}{}{}{}$ rule,
  ...
  in $b$ to occur in a subterm of the form $f\,a_1 \cdots a_n$ with
  $a_k \in V$.  We further require $f, \tup{x}, \tup{y}_i$ not to be
  shadowed in $b_i$.}
\label{fig:terms2}
\end{figure}
...
clause variables that occur verbatim among the result indices --- forced
variables in the sense of Brady et al.\ \cite{BradyEtAl2004} and Gilbert
et al.\ \cite{GilbertEtAl2019} --- and the remaining CIC constructor
arguments become the premises $\tup{\varphi}$.  Every CIC singleton
elimination is therefore an instance of $\tscase{}{}{}{}$.  The converse
fails: \cref{def:forced-singleton} does not require the forced variables
...
(\cref{sec:limitations-reconstruction}).  Empty elimination needs no new
...
\begin{align*}
  ...
  & \tscase{J}{\wcns{k}(\tup{a};\tup{\pi})}{\tup{z}.\tau}{\tup{p}.t} &\red
     \subst{t}{\tup{\pi}}{\tup{p}}
\end{align*}
together with the rule for case analysis meeting a constructor,
\begin{multline*}
...
\begin{proof}
Routine simultaneous induction on the second derivation, exactly as in
\cite{Extraction2026}: $\conv$ is closed under substitution (each root
rule is, including the new case and $\mathsf{scase}$ rules, whose
instantiated proofs and branches substitute componentwise), and
substitution into predicate atoms $\subst{X(\tup{a})}{P}{X} = P \cdot
\tup{a}$ is given by predicate application, which terminates because
predicate expressions cannot mention the variable being substituted
after $\alpha$-renaming.  The new rules (constructor, case,
$\mathsf{scase}$, $\mathsf{discr}$, $\tind{I}$) are homomorphic in all
their components; for $\mathsf{discr}$ note that substitution preserves
the constructor-headedness of both equation sides, and for
$\tscase{}{}{}{}$ that $\theta$ commutes with the ambient substitution
because the clause data ($\tup{u}^0$, $\jmath$) are closed declaration
components while $\tup{u}$ substitutes pointwise.
\end{proof}
