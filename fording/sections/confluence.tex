...
\item Clauses (3), (5), (7), and clause (6) with non-constructor
  ...
  either way.
\item Clause (6) with constructor-form scrutinee --- the parallel step
  is a congruence but the development fires the $\iota$-redex.  Here $e
  = \mathsf{case}(\wcns{c}_i(\tup{e}); \{\wcns{c}_j(\tup{x}_j)
  \Rightarrow b_j\}_j)$ and $e^{\ast} =
  \subst{b_i^{\ast}}{\tup{e}^{\ast}}{\tup{x}_i}$ by
  \cref{def:development}.  The step gives $e' =
  \mathsf{case}(e''; \{\wcns{c}_j(\tup{x}_j) \Rightarrow b'_j\}_j)$
  with $\wcns{c}_i(\tup{e}) \Rightarrow e''$ and $b_j \Rightarrow
  b'_j$; the scrutinee derivation can only be by clause (5) --- a
  constructor form is neither a variable, $\bx$, a constant, nor an
  application, so no other clause matches its shape --- hence $e'' =
  \wcns{c}_i(\tup{e}'')$ with $e_l \Rightarrow e''_l$ componentwise.
  The induction hypotheses give $e''_l \Rightarrow e_l^{\ast}$ and
  $b'_i \Rightarrow b_i^{\ast}$, so clause (9) applied to $e' =
  \mathsf{case}(\wcns{c}_i(\tup{e}''); \{\wcns{c}_j(\tup{x}_j)
  \Rightarrow b'_j\}_j)$ yields $e' \Rightarrow
  \subst{b_i^{\ast}}{\tup{e}^{\ast}}{\tup{x}_i} = e^{\ast}$.  (This is
  the redex-created-by-development configuration the Takahashi method
  is designed for; the symmetric configuration --- step fires the
  redex, development likewise --- is clause (9) below.)
