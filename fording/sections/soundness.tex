...
Throughout, the domain $\Dom$, the interpretations $\semr{\cdot}{}$,
$\pv{\cdot}$, $\kv{\cdot}$ and the structure $\ML$ are the ones
constructed in \cref{sec:semantics} over the \emph{full} fixed
environment $\Env$.  For a well-formed prefix $\Env'$ of $\Env$, the
\emph{environment hypothesis at $\Env'$} is:
\begin{itemize}[leftmargin=2em]
\item[(H$_{\Env'}$)] for every term definition $c \defeq t : \tau_c$
  in $\Env'$, $\cls{c} \in \semr{\tau_c}{}$; and for every lemma $c
  \defeq \pi : \varphi_c$ in $\Env'$, $\pv{\varphi_c} = 1$;
\end{itemize}
we write (H) for (H$_{\Env}$) (discharged in \cref{thm:env-validity}).
The fundamental lemma is stated relative to a prefix precisely so that
\cref{thm:env-validity} can establish (H) by induction on declaration
position without circularity: the lemma is applied to bodies typed
over the \emph{strict} prefix preceding the definition under scrutiny,
and only the hypothesis for that prefix is assumed.  The
parameterisation costs nothing: a derivation over $\Env'$ is verbatim
a derivation over $\Env$ (typing consults declarations by name, and
$\Env'$-conversion is contained in $\Env$-conversion, under which the
semantics is invariant); all semantic machinery ---
\cref{def:interp,def:model},
\cref{lem:sem-basic,lem:sem-subst,lem:conv-inv}, the rank apparatus
--- is defined once over the full $\Env$ and does not depend on any
(H$_{\Env'}$); and the proof below consults the hypothesis only in the
constant cases, for constants the derivation mentions, which lie in
$\Env'$.
...
Let $\Env'$ be a well-formed prefix of $\Env$ (possibly $\Env$ itself)
such that (H$_{\Env'}$) holds.  Let $\mathcal{D}$ be a guard
discipline over a context $\Gamma$ over $\Env'$, and $\rho
\vDash_{\mathcal{D}} \Gamma$.
\begin{enumerate}[leftmargin=2em]
\item If $\Gamma \vdash t : \sigma$ is derivable over $\Env'$ and $t$
  respects $\mathcal{D}$, then $\sem{t}_\rho \in \semr{\sigma}{\rho}$.
\item If $\Gamma \vdash \pi : \varphi$ is derivable over $\Env'$ and
  $\pi$ respects $\mathcal{D}$, then $\pvr{\varphi}{\rho} = 1$.
\end{enumerate}
\end{lemma}
...
discipline to subphrases.  Occurrences of disciplined variables
\emph{inside types, propositions and predicate expressions} incur no
semantic obligation and need no propagation.  Such occurrences do
exist --- if the codomain of $T_f$ is an index type, a branch body may
contain a well-formed annotation $\lambda x{:}\vect(f\,k\,w).\,\dots$
with $w \in V_f$, an applied occurrence of $f$ inside a type that
satisfies the guard condition --- but every semantic clause of
\cref{def:interp} consumes embedded terms only through the denotation
$\sem{\cdot}_\rho$, which is defined for \emph{every} valuation:
membership of $\rho(f)$ in a (rank-restricted) type interpretation is
consulted only where this lemma's conclusion demands it for a term or
proof judgement, never when a type or proposition is evaluated.  When
extending $\rho$ at bound variables we $\alpha$-rename so that
disciplined variables, $V_f$-variables and context variables are not
shadowed.
...
arise for it.  For a term constant --- declared in $\Env'$, since the
derivation is over $\Env'$ --- $\sem{c}_\rho = \cls{c} \in
\semr{\tau_c}{}$ by (H$_{\Env'}$), the $\delta$-rule and weakening;
for a lemma constant in (2), directly by (H$_{\Env'}$).
...
\begin{proof}
(1) We prove (H$_{\Env'}$) for every well-formed prefix $\Env'$ of
$\Env$, by induction on its length; (H) is the case $\Env' = \Env$.
The empty prefix is vacuous.  If $\Env'$ ends in a datatype or
predicate declaration, (H$_{\Env'}$) coincides with the induction
hypothesis.  If $\Env'$ ends in $c \defeq t : \tau_c$, the body $t$ is
typed over the strict prefix $\Env''$ preceding it, for which
(H$_{\Env''}$) holds by induction; \cref{lem:fundamental} at prefix
$\Env''$, with empty context, discipline and valuation, gives
$\sem{t}_{} \in \semr{\tau_c}{}$, and $\cls{c} = \sem{t}_{}$ by the
$\delta$-rule (a conversion of the full environment, so the classes
coincide).  Lemmas identically via part (2) of the fundamental lemma.
...
\emph{Predicate clause axioms}: let $\tup{d}$ be an assignment of
$\tup{x}$ under which the guards $\guard{\tau_{ji}}{x_i}$ and the
premise images $\F{\varphi_{jl}}$ all hold in $\ML$, and let $\rho_0 =
[\tup{x} \mapsto \tup{d}\,]$.  By \cref{lem:coincidence}, $d_i \in
\semr{\tau_{ji}}{\rho_0}$ for every $i$ and $\pvr{\varphi_{jl}}
{\rho_0} = 1$ for every $l$; for a recursive premise $J(\tup{v})$ the
latter reads $\sem{\tup{v}}_{\rho_0} \in \pv{J}$, i.e.\ membership at
some finite stage.  The stage construction of \cref{def:interp}, at
the successor of the maximum of those stages, puts
$\sem{\tup{u}_j}_{\rho_0}$ in $\pv{J}$, and \cref{lem:comp-adequacy}
identifies the denotations of the conclusion atom's arguments
$\compf(\erase{\tup{u}_j})$ with exactly this tuple; hence the
conclusion atom holds.

\emph{Predicate case-analysis axioms}: let $\tup{d} \in \Ihat{J}^{\ML}
= \pv{J}$, and let $s + 1$ be the least stage containing it.
Unfolding \cref{def:interp} yields a clause $j$ and a valuation
$\rho_0 = [\tup{x} \mapsto \tup{d}'\,]$ with $d'_i \in
\semr{\tau_{ji}}{\rho_0}$, true non-recursive premises,
recursive-premise tuples in $\pv{J}^{s} \subseteq \pv{J}$, and $\tup{d}
= \sem{\tup{u}_j}_{\rho_0}$.  Instantiate the existentials of the
$j$-th disjunct at $\tup{d}'$: the guard and premise conjuncts hold by
\cref{lem:coincidence}, and each index equation $z_h \feq
\compf(\erase{u_{jh}})$ holds because $d_h = \sem{u_{jh}}_{\rho_0}$
while \cref{lem:comp-adequacy} identifies the right-hand side's
denotation at the chosen witnesses with the same class.  These are the
inversion and constructor-typing arguments one level down: predicates
have no element, so the constructor equation is absent and the
element-free tuple plays the role of $(w, \tup{z})$.
\end{proof}
...
\begin{proof}
By \cref{lem:fundamental} at $\Env' = \Env$ (with (H) supplied by
\cref{thm:env-validity}(1)),
...
(2) Write $\mathsf{enc}$ for the displayed map.  \emph{Well-definedness
and injectivity}: the image element is a constructor term; injectivity
of $\mathsf{enc}$ follows from \cref{cor:discrimination}(1) by
induction on sequence length, together with the injectivity of $n
\mapsto \cls{\csS^n(\csO)}$ (also \cref{cor:discrimination}(1)).
\emph{Image in $\semr{\vect}{}$}: by induction on $n$ using
...
\cls{\csvnil} = \mathsf{enc}(\varepsilon)$.  If by $\csvcons$ with
components $\cls{e_n}, \cls{e_a}, \cls{e_w}$: the recursive condition
gives $(\cls{e_n}, \cls{e_w}) \in \semr{\vect}{}$ (the index tuple of
the recursive argument is $(\sem{n}_{\rho_0}) = (\cls{e_n})$), so by
the inner induction $\cls{e_w} = \mathsf{enc}(a_2, \dots,
a_n)$-image with $\cls{e_n} = \cls{\csS^{n-1}(\csO)}$; moreover
$\cls{e_a} \in \semr{A}{}$, and $\tup{d} = (\sem{\csS\,n}_{\rho_0}) =
(\cls{\csS^{n}(\csO)})$; hence $(\tup{d}, d) =
\mathsf{enc}(\cls{e_a}, a_2, \dots, a_n)$.  The bijection $\mathbb{N} \cong \semr{\nat}{}$ is the
...
evident extension of $\ML$ --- re-run the construction of
\cref{sec:semantics} over the term language enlarged by the constant
$c$ with no $\delta$-rule; constructor and compiled-symbol
interpretations are given by the same clauses with the same witnesses,
$c$ itself denotes its own class $\cls{c}$, and every validity
argument is unaffected, none of them consulting the absence of extra
constants).  Excluded middle and proof irrelevance are
