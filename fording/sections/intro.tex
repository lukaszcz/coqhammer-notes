...
\item We define the translation (\cref{sec:translation}).  Guard
  ...
  compile, unchanged, to unguarded per-constructor equations.  We prove
...
\item We settle the indexed-specific design questions
  ...
  \begin{itemize}
  \item the inversion axiom's guard is mandatory in a strong sense ---
    for every family that specialises an index to a constructor form,
    the unguarded axiom is \emph{inconsistent} with the discrimination
    axioms, not merely unsound
    (\cref{prop:unguarded-inconsistent});
