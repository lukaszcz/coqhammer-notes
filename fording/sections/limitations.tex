...
completeness claim that would force them.  The same holds for letting
the prover generate induction instances internally, in the
saturation-with-induction style
\cite{RegerVoronkov2019,HajduEtAl2022}: the guarded first-order
induction schema of a declared family is true in $\ML$ --- the
datatype-induction case of \cref{lem:fundamental} is exactly its
semantic form --- so such prover-side extensions compose soundly with
this translation.  Completeness fails further
...
\cite{VazouEtAl2014}.  A completeness theorem for the \emph{ground
equational} fragment over purely structural families --- if $\pv{a
=_\sigma b} = 1$ for closed $a, b$ of purely structural type then
$\ThE \provesfol \compf(\erase{a}) \feq \compf(\erase{b})$ --- is
plausibly within reach, but it is not a corollary of what is proved
here, and we state the missing piece exactly rather than gesture at
``standard rewriting arguments''.  \Cref{rem:eq-shape} makes the
emitted equations a non-overlapping rewrite system (orthogonal in the
absence of recursive closures) and \cref{prop:standard}
makes $\ML$ standard on the relevant types; what is missing is a
simulation-and-standardisation development relating that rewrite
system to $\lbox$ conversion on compiled images.  The hypothesis
$\pv{a =_\sigma b} = 1$ says the \emph{erasures} are
$\lbox$-convertible; this paper proves no normalisation or
standardisation theorem for $\lbox$ (none is needed anywhere else),
and conversion steps under the binders of lifted symbols are precisely
what the emitted equations do not reflect.  For ground terms of
structural type the under-binder steps should be avoidable along a
weak-head standard reduction, so we expect the theorem to hold; proving
that simulation is the actual content of the folklore argument, and we
...
equality is bisimilarity --- are a genuine further step.  Two further
restrictions of the declaration format (\cref{def:env}) deserve
explicit listing.  First, recursive occurrences must be direct:
argument types of the form $\sigma \to I(\tup{v})$ --- function-space
recursive arguments, infinitary branching, W-types --- are excluded;
only first-order recursive positions $I(\tup{v})$ are admitted.
Second, all propositional premises of a constructor come after all
informative arguments; a CIC constructor in which the \emph{type} of a
later informative argument mentions an earlier propositional argument
(a proof subterm inside an index term of a subsequent argument type)
has no direct image in the format.  Under proof irrelevance the
reordering is semantically harmless --- the premise proofs are erased
and the index terms lose their proof subterms either way
(\cref{rem:proof-indices}) --- but it is a genuine syntactic
restriction of the fragment.  Enumeration-,
