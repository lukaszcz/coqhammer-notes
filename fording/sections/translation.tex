...
clause costs one line and makes every totality statement in this
section (\cref{lem:compile-term}, the totality of $\guardf$ and $\Ff$
in \cref{def:guard-F}) unconditional.
\end{remark}
...
emits axioms; throughout, $\forall.\ \phi$ denotes the universal
closure of $\phi$ (all free first-order variables quantified):
...
\item If $e = \mathsf{case}(s;\, \tup{br})$ with $s$ not a variable of
  ...
  emit $\forall.\ L \feq g(\compf_\theta(s), \tup{w}')$, and run
...
$L \feq R$ whose left-hand sides are \emph{linear} terms of the form
\[
  f(\tup{\pi}) \fapp \pi'_1 \fapp \cdots \fapp \pi'_k \qquad (k \geq 0),
\]
where $f$ is a compiled symbol and each of $\tup{\pi}, \pi'_1, \dots,
\pi'_k$ is a variable or a constructor pattern.  Note that constructor
patterns occur in $\fapp$-positions too: rule (2) of \cref{def:compile}
may refine a variable that rule (1) appended --- a branch body that is
a $\lambda$-abstraction followed by a case on the new variable ---
producing left-hand sides such as $f(\Ihat{\csO}) \fapp
\Ihat{\wcns{c}}(\tup{z})$.  The left-hand sides are pairwise
non-overlapping: the equations emitted for one head symbol are the
leaves of a \emph{refinement tree} in which rule (1) wraps the current
left-hand side $L$ in one $\fapp$-application of a fresh variable,
rule (2) splits one variable of $L$ into all constructors of the
scrutinised datatype, and rules (3)--(5) emit and stop.  Two distinct
leaves diverge at a rule-(2) split of some variable $x$ of a common
intermediate left-hand side $L_0$; in the two final left-hand sides,
$L_0$ persists as the subterm at a fixed depth on the left
$\fapp$-spine (later steps only wrap it in further $\fapp$-applications
or instantiate its variables), and the position of $x$ carries an
instance of $\Ihat{\wcns{c}}(\cdots)$ in one and of
$\Ihat{\wcns{d}}(\cdots)$ in the other with $\wcns{c} \neq \wcns{d}$,
so no unifier exists between either final left-hand side and the other
or a left-spine subterm of the other.  Left-hand sides with distinct
head symbols cannot unify at all (the head symbol sits at the bottom of
the left spine), and each pointer axiom is the sole equation of its
head $f^0$, with an all-variable left-hand side that cannot unify with
a proper left-spine subterm of a renamed copy of itself
($\fapp$-depths differ).  Left-linear and non-overlapping, the
equations oriented left to right form an orthogonal rewrite system
\cite{BaaderNipkow1998}.  Nothing about indexed families disturbs
this: the patterns are patterns of \emph{informative} constructor
arguments, and index information never reaches $\lbox$.
\end{remark}
