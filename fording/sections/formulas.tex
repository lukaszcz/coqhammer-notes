...
\F{\varphi_0}$ --- from the atomic translation:
...
\begin{proof}
(1) Take fresh variables $w, \tup{z}, \tup{z}'$ and the two instances
$\mathrm{Inv}^-_I(w, \tup{z})$ and $\mathrm{Inv}^-_I(w, \tup{z}')$ ---
the \emph{same} element variable, two arbitrary index tuples.  Reason
by cases on both disjunctions.  A case pairs a disjunct $i$, with
